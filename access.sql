...
--7
UPDATE parks SET p_latitude = 1.00, p_longitude = 1.00
WHERE p_park_code = 'park code';

--Species Info
--8
INSERT INTO species (s_species_id, s_park_name, s_category, s_order, s_family, s_occurence, s_nativeness, s_abundance, s_seasonality)
...
--Animal Info
--11
INSERT INTO animals (a_species_id, a_scientific_name, a_record_status, a_conservation_status)
VALUES ('species id', 'scientific name', 'record status', 'conservation status');

--12
UPDATE animals SET a_record_status = 'record status', s_conservation_status = 'conservation status'
...
--15
UPDATE plants SET pl_record_status = 'record status', s_conservation_status = 'conservation status'
...
--Search Saving
--17
INSERT INTO search (se_search_term, se_park_name)
VALUES ('search term', 'park name');

--18
DELETE FROM search
WHERE se_search_term = 'search term';
...
WHERE a_scientific_name = 'search term'
OR cn_common_name = 'search term'
...
WHERE pl_scientific_name = 'search term'
OR cn_common_name = 'search term'
AND s_species_id = pl_species_id
ORDER BY pl_scientific_name ASC;

--24
SELECT a_scientific_name, s_occurance, s_nativeness, s_abundance, s_seasonality, a_record_status, a_conservation_status
FROM animals, species
WHERE a_scientific_name = 'scientific name'
AND s_species_id = a_species_id
ORDER BY a_scientific_name ASC;

--25
SELECT pl_scientific_name, s_occurance, s_nativeness, s_abundance, s_seasonality, pl_record_status, pl_conservation_status
FROM plants, species
WHERE pl_scientific_name = 'scientific name'
AND s_species_id = pl_species_id
ORDER BY pl_scientific_name ASC;
